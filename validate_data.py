...
def test_field_DataFrame_non_negative_elevation():
    ...
    field_data = field_processor.df
    assert field_data[field_data[['Elevation']] >= 0].shape == (5654, 19)
...
def test_crop_types_are_valid():
    ...
    field_processor.apply_corrections()
    field_data = field_processor.df
    assert list(field_data['Crop_type'].unique()) == ['cassava', 'tea', 'wheat', 'potato', 'banana', 'coffee', 'rice', 'maize']
